...
# Convert raw data to g and dps
    def convertDate(self,msg):
        ...
        accel_z_filtered = msg.acz

        gyro_x_filtered = (msg.gx)
        gyro_y_filtered = (msg.gy)
        gyro_z_filtered = (msg.gz)

        accel_x_filtered2 = (msg.acx2)
        accel_y_filtered2 = (msg.acy2)
        accel_z_filtered2 = (msg.acz2)

        gyro_x_filtered2 = (msg.gx2)
        gyro_y_filtered2 = (msg.gy2)
        gyro_z_filtered2 = (msg.gz2)
    ...
    def calibrate_mpu(self, num_samples=10000):
        ...
        if len(self.acc_buffers['acx'])>= self.window_size :
            ...
            gyro_data_yaw = np.array(self.gyro_buffers['yaw'])
    ...
    def calculateCalibrations(self,accel_data,gyro_data,key):
        ...
        """      
        self.get_logger().info(f"Accel standard deviation: {accel_std},Accel mean: {accel_mean}, Gyro standard deviation: {gyro_std},Gyro mean: {gyro_offset}")

        self.get_logger().info(f"Calibration completed for {key}. Accel offsets: {accel_offset}, Gyro offsets: {gyro_offset}")

        # Print the last 15 values of the arrays
        self.get_logger().info(f"Last 15 accel data for {key}: {accel_data_array[-15:]}")
        self.get_logger().info(f"Last 15 gyro data for {key}: {gyro_data_array[-15:]}")
        """
    ...
    def read_sensor_data(self, accel, gyro, calibration_key,prev):
        """Reads the  sensor data from the MPU9250 after kalman filtering and calibration."""
        accel_x =accel[0]
        accel_y = accel[1]
        accel_z = accel[2]
        gyro_x = gyro[0]
        ...
        log_data = f"Before calibration: KEY {calibration_key} {round(accel_x,3)}, {round(accel_y,3)}, {round(accel_z,3)}, Gyro: {round(gyro_x,3)}, {round(gyro_y,3)}, {round(gyro_z,3)}"
        ...
        """
        accel_x = (accel_x - calibration_data[calibration_key]["accel"]["offset"][0]) 
        accel_y = (accel_y - calibration_data[calibration_key]["accel"]["offset"][1]) 
        accel_z = (accel_z - calibration_data[calibration_key]["accel"]["offset"][2]) 
        ...
        if show:
            ...
            self.get_logger().info(f"Adaptive calibration adjustment for {calibration_key}: "
                                f"Accel X offset: {round(calibration_data[calibration_key]['accel']['offset'][0],3)}, "
                                f"Accel Y offset: {round(calibration_data[calibration_key]['accel']['offset'][1],3)}, "
                                f"Accel Z offset: {round(calibration_data[calibration_key]['accel']['offset'][2],3)}")
            self.get_logger().info(f"Adaptive calibration adjustment for {calibration_key}: "
                                f"Accel X slope: {round(calibration_data[calibration_key]['accel']['slope'][0],3)}, "
                                f"Accel Y slope: {round(calibration_data[calibration_key]['accel']['slope'][1],3)}, "
                                f"Accel Z slope: {round(calibration_data[calibration_key]['accel']['slope'][2],3)}")
            self.get_logger().info(f"Adaptive calibration adjustment for {calibration_key}: "
                                f"gyro X offset: {round(calibration_data[calibration_key]['gyro']['offset'][0],3)}, "
                                f"gyro Y offset: {round(calibration_data[calibration_key]['gyro']['offset'][1],3)}, "
                                f"gyro Z offset: {round(calibration_data[calibration_key]['gyro']['offset'][2],3)}")
